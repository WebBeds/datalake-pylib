...
class WrapperMetrics:
    ...
    def send(self):
        ...
        logging.debug(
            "sending metrics to cloudwatch {0} namespace with region {1}".format(
                self.namespace, self.aws_region
            )
        )
    ...
    def __str__(self) -> str:
        return "WrapperMetrics({0}, {1}, {2}, {3})".format(
            self.namespace, self.team, self.group, self.aws_region
        )
