...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="SQLMonitor, monitor your SQL query, send messages to CloudWatch metrics or any other metrics system"
    )
    ...
    parser.add_argument("-m", "--monitors", help="monitors to run", default=None, type=str)
...
def parse_config(config_file: str) -> configparser.ConfigParser:
    ...
    return {
        key: {arg.key: arg.parse(arg.key, config[key] if config else None) for arg in keys[key]}
        for key in keys
    }
...
def parse_monitor(monitor_file: str, conf: argparse.Namespace) -> list:
    ...
    if not isinstance(spec, dict) and not isinstance(spec, list):
        raise Exception(
            'Monitor file "{0}" is not a valid list or dict data structure'.format(monitor_file)
        )
